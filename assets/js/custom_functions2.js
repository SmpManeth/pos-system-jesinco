...
//                        "border-radius": "3px",
    ...
    Overlay_input: function (config) {
        ...
        if (type.toLowerCase() === "select") {
            ...
            for (var ii = 0; ii < i_options.length; ii++) {
                var opt = document.createElement("option");
                $(opt).html(i_options[ii].html).val(i_options[ii].val);
                $(input).append(opt);
            }
        } else {
        ...
        $(btn).attr({type: "button"}).html(options.button.yes.txt).addClass("btn-block btn btn-success");
