...
<!--                    <input type="hidden" value="" name="id" id="id" />-->
                    <div class="form-body">
                        <div class="form-group well">
                                    ...
                                    <optgroup>
                                        ...
                                        if (isset($customers)) {
                                            foreach ($customers as $cus) {
                                                ?>
                                                <option data-subtext="<?php echo $cus->devision ?>" value="<?php echo $cus->id ?>" <?php echo $cus->id == $invoice->customer ? "selected" : "" ?>> <?php echo $cus->customer_prefix . " " . $cus->customer_name ?></option>
                                                <?php
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="text" class="form-control input-sm" name="do_number" id="do_number" value="<?php echo $invoice->do_number ?>" />
                            </div>
                        </div>
                            ...
                            <table class="table table-striped table-bordered ">
                                ...
                                <tbody id="item_body">
                                    ...
                                    if (isset($invoice_items)) {
                                        $i = 0;
                                        $can_edit_do = user_can($user, CAN_EDIT_DO);
                                ...
                                <tfoot>
                                    ...
                                    <tr>
                                        ...
                                        <td class="" style="width:150px;">
                                            <input type="hidden"  id="down_payment" class="form-control" value="0" />
                                            <input type="text" readonly  id="serive_charge" class="form-control text-right number" value="<?php echo get_option('service-charge',null,$branch->id) ?>" />
                                        </td>
                                        <td></td>
                                    </tr>
                                    <tr>
                    ...
                    <div class="row well">
                        <div class="col-lg-12 text-right col-sm-12">
                            <?php
                            if (user_can($user, CAN_APPROVE_DO)) {
                                ?>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#cancel_po_btn").click(function () {
            DJ.Confirm("Do You want to Cancel this Delivery Order?", function () {
                var id = $("#inv_id").val();
                ...
                $.post("<?php echo base_url("reservation/cancel_reservation_note"); ?>", data, function (data) {
...
//                });
                ...
                DJ.Overlay_confirm({
                    title: "Do You want to Finish this Delivery Order?",
                    button: {
                        yes: {txt: "Proceed"},
                        no: {txt: "NO"}
                    },
                    click: function (v) {
                        ...
                        if (v) {
                            DJ.disable_btn_fa("finish_as_reservation", "Processing");
                            var id = $("#inv_id").val();
                            ...
                            $.ajax({
                                url: "<?php echo base_url("reservation/make_reservation") ?>",
                                type: 'POST',
                                dataType: 'JSON',
                                data: {id: id, do_number: do_number, customer: customer, inv_date: inv_date, remarks: remarks},
                                success: function (data) {
                                    console.log(data);
                                    DJ.enable_btn_fa("finish_as_reservation", "Finish Reservation");
                                    if (data.msg_type == "OK") {
                                        DJ.Notify(data.msg, "success");
                                        location.href = data.url;
                                    } else {
            ...
            if ($("#save_discount").is(":visible")) {
                ...
                DJ.Confirm("Do You want to Finish this Invoice?<br/><strong>Invoice will Finished as Full Payment Received.<strong>", function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            DJ.enable_btn_fa("finish_inv_btn", "Finish Cash");
                            if (data.msg_type == "OK") {
                                DJ.Notify(data.msg, "success");
                                location.href = data.url;
                            } else {
    ...
    function calculate_discount() {
        ...
        var tot = Number(sub_totla) - Number(service_charge);
