...
<title>Edit Good Receive Note</title>
<?php
?>
...
<!--                    <input type="hidden" value="" name="id" id="id" />-->
                    <div class="form-body">
                        <div class="form-group well">
                            <div class="col-md-4">
                                ...
                                <input type="hidden" value="<?php echo $grn->id ?>" name="id" id="gr_id" />
                                <select class="form-control btn-sm select-picker" name="supplier" id="supplier" <?php echo!empty($grn->po_id) ? "disabled=''" : "" ?>>
                                    <optgroup>
                                    ...
                                    <optgroup>
                                        ...
                                        if (isset($sups)) {
                                            foreach ($sups as $sup) {
                                                ?>
                                                <option <?php echo $grn->supplier == $sup->id ? "selected" : "" ?> data-subtext="<?php echo $sup->bis_type ?>" value="<?php echo $sup->id ?>"><?php echo $sup->company_name ?></option>
                                                <?php
                                            }
                                        }
                                        ?>
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="text" class="form-control input-sm input-sm datepicker input-sm" name="po_date" id="po_date" value="<?php echo $grn->grn_date ?>"/>
                            </div>
                            <div class="col-md-2">
                                <label class="control-label">PO Reference :</label>
                                <input type="text" class="form-control input-sm" name="po_ref" id="po_ref" <?php echo!empty($grn->po_id) ? "readonly=''" : "" ?>  value="<?php echo $grn->po_ref ?>"/>
                            </div>
                            <div class="col-md-4">
                                <label class="control-label">Delivery Location :</label>
                                <input type="text" class="form-control input-sm" name="del_location" id="del_location"  value="<?php echo $grn->del_location ?>"/>
                            </div>
                        </div>
                            ...
                            <table class="table table-striped table-bordered ">
                                        ...
                                        foreach ($g_items as $g_item) {
                                            if ($g_item->is_temp != "1" && $g_item->foc == "0") {
                                                ...
                                                <tr id="grn_<?php echo $g_item->id ?>">
                                                    <td><?php echo $g_item->itm_name ?>&nbsp;&nbsp;<span class="small text-muted"><?php echo $g_item->itm_code ?></span></td>
                                                    <td><span><?php echo $g_item->qty ?></span><i class="fa fa-edit edit_qty_btn" data-id="<?php echo $g_item->id ?>"></i></td>
                                                    ...
                                                    <td>
                                                        <a onclick="remove_item_po(<?php echo $g_item->id ?>)"><i class="fa fa-remove font-red-thunderbird"></i></a>
                                                        <?php
                                                        if ($g_item->unique_type == "1") {
                                                            ?>
                                                            <a onclick="add_serials('<?php echo $g_item->grn_id ?>', '<?php echo $g_item->item_id ?>', '<?php echo $g_item->id ?>', this)"><i class="fa fa-barcode"></i></a>
                                                            <?php
                                                        }
                                                        ?>
                                                    </td>
                                ...
                                <tfoot>
                                    ...
                                    <tr>
                                        ...
                                        <td id="sub_total" class="text-strong text-right"><?php echo $grn->sub_total ?></td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3" class="text-right">Discount</td>
                                        <td class="text-strong text-right" style="width:150px;">
                                            <input type="text" onkeyup="calculate_discount();" class="form-control number text-right" id="discount" name="discount" value="<?php echo $grn->discount ?>" />
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td id="gr_total" class="text-strong text-right"><?php echo $grn->total ?></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>`
                    ...
                    <div class="row well">
                        ...
                        <div class="col-lg-12 text-right col-sm-12">
                            ...
                            <button type="button" class="btn btn-warning btn-sm" id="cancel_po_btn"><i class="fa fa-remove text-danger"></i> <span>Cancel</span></button>
                            <a href="<?php echo base_url("grn") ?>" title="Save and Go Back" class="btn default btn-sm" id="cancel_po_btn"><i class="fa fa-arrow-left"></i> Go Back</a>
                        </div>
                    </div>
                    <?php echo form_close() ?>
                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $(".edit_qty_btn").click(function () {
            var ele = this;
            var qty = $(ele).parent().find("span:first").html();
            var id = $(ele).data("id");
            open_qty_editor(ele, id, qty);
        });
        ...
        $("#pre-rec-items").click(function () {
            DJ.load_to_model({
                ...
                url: "<?php echo base_url("grn/get_pre_rec_items") ?>",
                type: 'modal-lg',
                fade: ''
            });
        });
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.msg_type == "OK") {
                        ...
                        $(serial).html("<i class='fa fa-barcode'></i>").attr({href: "javascript:;"}).click(function () {
                            add_serials(data.p_id, data.itm_id, data.pi_id, a);
                        });
                        $(edit_qty).addClass("fa fa-edit edit_qty_btn").click(function () {
                            open_qty_editor(edit_qty, data.pi_id, qty);
                        });
                        $(td2).html("<span>" + qty + "</span>").append(edit_qty);
                        if (foc == '1') {
                            $(td3).html(0.00);
